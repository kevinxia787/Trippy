  ...
  renderUsers = () => {
    ...
    return Users.map((item, i) => {
      if ( i < this.state.currentIndex) {
        ...
        return (
          /**
           * We will need to update this view to be image of the venue recommendation
           * as well as the address as a text.
           */
          <Animated.View 
          {...this.PanResponder.panHandlers} 
          key={item.id} style={[ this.rotateAndTranslate, {height: SCREEN_HEIGHT - 120, width: SCREEN_WIDTH, padding: 10, position: 'absolute'}]}>
            <Animated.View style={{opacity: this.likeOpacity, transform: [{rotate: '-30deg'}], position: 'absolute', top: 50, left: 40, zIndex: 1000 }}>
              <Text style={{ borderWidth: 1, borderColor: 'green', color:'green', fontSize: 24, fontWeight: '800', padding: 10 }}>LET'S GO</Text>
            </Animated.View>
            <Animated.View style={{ opacity: this.nopeOpacity, transform: [{rotate: '30deg'}], position: 'absolute', top: 50, right: 40, zIndex: 1000 }}>
              <Text style={{ borderWidth: 1, borderColor: 'red', color:'red', fontSize: 24, fontWeight: '800', padding: 10 }}>NOPE</Text>
            </Animated.View>
